...
import { checkSuspensionExpiry, getSuspensionInfo } from "../utils/suspensionUtils.js";
    ...
    if (store) {
      ...
      if (currentUser.accountStatus === "suspended") {
        ...
        const message = suspensionInfo.daysRemaining
          ? `Your account has been suspended for ${suspensionInfo.daysRemaining} days ${suspensionInfo.hoursRemaining % 24} hours. Reason: ${suspensionInfo.reason || "Account suspended by admin"}`
          : `Your account has been permanently suspended. Reason: ${suspensionInfo.reason || "Account suspended by admin"}`;
